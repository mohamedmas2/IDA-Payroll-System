...
# --- 3. CSS الاحترافي (لفرد المحتوى ومنع التكدس في النص) ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;800&display=swap');
    
    /* منع التوسيط الإجباري وفرد الصفحة */
    ...
    html, body, [class*="css"] { 
        ...
        text-align: right !important; /* إجبار المحتوى على اليمين */
    }

    /* تحسين شكل اللوجو والعناصر في الموبايل */
    [data-testid="stImage"] {
    ...
    .custom-table th { background-color: #003366; color: white; padding: 10px; text-align: center; }
    .custom-table td { padding: 8px; border: 1px solid #eee; text-align: center; font-weight: 600; }

    /* 📱 ضبط الموبايل الخاص لمنع "العرض الطولي" */
    ...
    /* 🖨️ الطباعة النظيفة */
    @media print {
        section[data-testid="stSidebar"], header, [data-testid="stHeader"], .stButton { display: none !important; }
...
if not st.session_state["logged_in"]:
    ...
    # --- 5. منطقة العرض المفرودة (Main Content) ---
    if df_raw is not None:
        df_f = df_raw if t_month == "الكل" else df_raw[df_raw[cols['date']].astype(str) == t_month]
        
        if menu == "🔍 استعلام":
            # إجبار العناصر على عدم التكدس في النص عبر كتابة الكود بشكل مباشر بدون حاويات (st.columns)
            ...
            if q:
                ...
                if not res.empty:
                    for n, gp in res.groupby(cols['name']):
                        st.markdown(f'<div class="personal-card"><h1>{n}</h1><p>🆔 كود: {gp.iloc[0][cols["code"]]} | 📄 قومي: {gp.iloc[0][cols["nat"]]}</p></div>', unsafe_allow_html=True)
