...
# 1. إعداد الصفحة (أول أمر)
st.set_page_config(page_title="نظام IDA للمستحقات", layout="wide", page_icon="💰")

# 2. تصميم CSS (الشكل القديم الفخم مع معالجة الموبايل والطباعة)
st.markdown("""
    <style>
    ...
    /* كروت البيانات الكبيرة (الشكل القديم) */
...
def load_data_v43():
    ...
    try:
        df = pd.read_csv(f, header=0, encoding='utf-8-sig', low_memory=False, dtype={'National_ID': str, 'Employee_Code': str, 'NON': str})
...
df_raw, cols = load_data_v43()

if df_raw is not None:
    # القائمة الجانبية (نظيفة تماماً من العناوين)
    with st.sidebar:
        if cols['date']:
    ...
    # 1. شاشة الاستعلام (نفس الشكل القديم)
    ...
    # 3. تصدير التقارير (إصلاح الخطأ)
        ...
        try:
            ...
            st.download_button("💾 تحميل Excel الشامل", buffer.getvalue(), f"IDA_{target_month}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        except Exception as e:
            st.error(f"يرجى التأكد من وجود ملف requirements.txt على GitHub. الخطأ: {e}")
