...
def load_v40_data():
    ...
    try:
        # قراءة البيانات مع إجبار التاريخ والكود على أن يكونوا نصوصاً
        ...
        # تنظيف عمود التاريخ من أي علامات عشرية (392026.0 -> 392026)
        if cols['date']:
            df[cols['date']] = df[cols['date']].astype(str).str.replace(r'\.0$', '', regex=True).str.strip()
            # استبدال nan بنص فارغ
            df[cols['date']] = df[cols['date']].replace('nan', 'غير محدد')
...
if df_raw is not None:
    with st.sidebar:
        ...
        if cols['date']:
            ...
            all_dates = sorted(df_raw[cols['date']].unique().tolist(), reverse=True)
            available_months = ["الكل"] + all_dates
            target_month = st.selectbox("📅 اختر شهر الصرف:", available_months)
            
            # منطق الفلترة (لو اختار الكل ميعملش فلترة)
        ...
        if q:
            ...
            if not res.empty:
                for name, group in res.groupby(cols['name']):
                    ...
                    # في حالة "الكل"، بنعرض عمود التاريخ في الجدول عشان الموظف يعرف كل سطر تبع أنهي شهر
