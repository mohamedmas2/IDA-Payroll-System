...
if df_raw is not None:
    ...
    if menu == "🔍 استعلام الموظفين":
        ...
        if q:
            ...
            # نبحث في النسخة المفلترة حسب الشهر
            res = df_filtered[(df_filtered['Search_Key'].str.contains(q_clean, na=False)) | 
                              (df_filtered[cols['code']].astype(str).str.contains(q.strip()))]
            ...
            if not res.empty:
                for name, group in res.groupby(cols['name']):
                    ...
                    # الجدول (بإضافة عمود الشهر لو "الكل")
        ...
        st.metric("صافي المنصرف للفترة المختارة", f"{df_filtered[cols['net']].sum():,.2f}")
