...
# 1. إعداد الصفحة (باستخدام اللوجو الخاص بك)
st.set_page_config(page_title="نظام IDA للمستحقات", layout="wide", page_icon="IDA_logo_(1).ico")

# 2. تصميم CSS (التنسيق القديم الفخم اللي بتحبه بالظبط)
st.markdown("""
    <style>
    ...
    /* كروت الإحصائيات الملونة (القديمة المعتمدة) */
...
if df_raw is not None:
    ...
    # --- شاشة الاستعلام (التنسيق القديم الأصلي) ---
        ...
        if q:
            ...
            if not res.empty:
                for name, group in res.groupby(cols['name']):
                    ...
                    s_tax = group[cols['tax']].sum() + group[cols['stamp']].sum() # التعديل المطلوب هنا
                    ...
                    # الجدول (بدون أرقام صفوف)
                    d_df = group[[cols['type'], cols['desc'], cols['ent'], cols['net']]].copy()
                    d_df.insert(0, 'م', range(1, len(d_df)+1))
                    st.markdown(f"<div class='custom-table-container'>{d_df.to_html(index=False, classes='custom-table')}</div>", unsafe_allow_html=True)
