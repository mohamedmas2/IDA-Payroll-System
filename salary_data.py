...
# 2. تصميم CSS (العودة للخطوط الكبيرة والشكل القديم الفخم)
st.markdown("""
    <style>
    ...
    .app-main-title { color: #003366; font-size: 30px; font-weight: 800; margin-bottom: 25px; text-align: center; border-bottom: 3px solid #003366; display: inline-block; padding-bottom: 10px; }

    /* كروت البيانات الكبيرة الواضحة */
    ...
    /* كروت الإحصائيات الملونة (القديمة) */
    ...
    /* الجداول الواضحة (بدون أرقام صفوف) */
    ...
    @media print {
        section[data-testid="stSidebar"], .stDownloadButton, button, iframe, header, [data-testid="stHeader"], .stTextInput, .stSelectbox, .stHeader, h1:first-of-type { display: none !important; }
        ...
        .stat-card { border: 1px solid #ddd !important; background-color: #eee !important; }
...
if df_raw is not None:
    ...
    if menu == "🔍 استعلام الموظفين":
        st.title(f"🔍 مستحقات فترة: {target_month}")
        ...
        if q:
            ...
            if not res.empty:
                for name, group in res.groupby(cols['name']):
                    ...
                    i1.markdown(f'<div class="stat-card" style="background:#28a745;"><span class="stat-label">إجمالي المستحق</span><span class="stat-value">{s_ent:,.2f}</span></div>', unsafe_allow_html=True)
                    i2.markdown(f'<div class="stat-card" style="background:#ffc107;"><span class="stat-label" style="color:black">ضرائب ودمغة</span><span class="stat-value" style="color:black">{s_tax:,.2f}</span></div>', unsafe_allow_html=True)
                    i3.markdown(f'<div class="stat-card" style="background:#dc3545;"><span class="stat-label">إجمالي استقطاع</span><span class="stat-value">{s_ded:,.2f}</span></div>', unsafe_allow_html=True)
                    i4.markdown(f'<div class="stat-card" style="background:#007bff;"><span class="stat-label">الصافي النهائي</span><span class="stat-value">{s_net:,.2f}</span></div>', unsafe_allow_html=True)
                    
                    # الجدول (إخفاء أرقام الصفوف وإضافة مسلسل)
        ...
        st.subheader(f"📊 مؤشرات الأداء - {target_month}")
        k1, k2, k3 = st.columns(3)
        k1.metric("عدد الموظفين", f"{df['Search_Key'].nunique():,}")
        k2.metric("إجمالي الصرف", f"{df[cols['ent']].sum():,.0f}")
        k3.metric("صافي السيولة", f"{df[cols['net']].sum():,.0f}")
        st.markdown("---")
